...
const waitingTickets = () =>
  store.tickets.filter((t) => t.status === "waiting" || t.status === "called");
...
function setRoute(route) {
  ...
  ["join", "status", "staff", "analytics"].forEach((r) => {
    const page = $("#page-" + r);
...
$$(".nav-btn").forEach((btn) =>
  btn.addEventListener(
    "click",
    () => btn.dataset.route && setRoute(btn.dataset.route)
  )
);
...
async function renderStatus() {
  ...
  if (!tid) {
    ...
    $("#queue-list").innerHTML =
      "<p class='subtle'>No ticket selected yet. Join the queue to see your status.</p>";
  ...
  if (!me) {
    ...
    $("#your-eta").textContent = "-";
    $("#queue-list").innerHTML =
      "<p class='subtle'>Your ticket is not in the active queue. It may have been served or cancelled. Join again if needed.</p>";
    $("#leave-queue").style.display = "none";
    return;
  }
  ...
  $("#leave-queue").style.display =
    me.status === "waiting" || me.status === "called" ? "inline-block" : "none";
    ...
    list.forEach((x, i) => {
      ...
      li.innerHTML = `
        <div class="row">
          ...
          ${
            x.status === "called"
              ? '<span class="pill" style="background:#dcfce7;color:#166534">called</span>'
              : ""
          }
        </div>
        <div class="micro subtle">${new Date(
          x.created_at
...
async function renderStaff() {
  ...
  let queue = [],
    ...
    summary = {};
  try {
    queue = await api(`/api/tickets?locationId=${LOCATION_ID}`);
  } catch (e) {
    console.error("queue load failed:", e);
  }
  try {
    settings = await api(`/api/settings/${LOCATION_ID}`);
  } catch (e) {
  ...
  try {
    ...
    const d = new Date();
    const localDate = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(
      d.getDate()
    )}`;
    summary = await api(
      `/api/analytics/summary/${LOCATION_ID}?date=${localDate}`
    );
  } catch (e) {
    ...
    queue.forEach((t, i) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          ${
            t.status === "waiting"
              ? '<button class="btn tiny" data-act="call">Call</button>'
              : ""
          }
...
$("#input-message").addEventListener("change", (e) => {
  store.customMsg = e.target.value;
  save();
  render();
});
$("#input-avg").addEventListener("change", (e) => {
  store.avgServiceMins = Math.max(1, Number(e.target.value || 1));
  save();
  render();
});
...
async function renderAnalytics() {
  ...
  try {
    ...
    const data = await api(
      `/api/analytics/served-per-hour/${LOCATION_ID}?date=${localDate}`
    );
    chart.innerHTML = "";
